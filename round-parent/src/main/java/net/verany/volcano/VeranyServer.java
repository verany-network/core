        ...
        AbstractVerany.addTask(new AbstractTask(1000) {
            ...
            public void run() {
                ...
                for (AbstractVolcanoRound round : ROUNDS) {
                    if (round.getSettingValue(net.verany.volcano.round.GameSetting.GAME_STATE).equals(GameState.WAITING))
                        lobbyRounds++;
                    ...
                    double percentage = round.getSettingValue(net.verany.volcano.round.GameSetting.START_WHEN_FULL);
                    if (percentage == -1) continue;
                    double online = round.getPlayers().size() * 100D / round.getSettingValue(net.verany.volcano.round.GameSetting.MAX_PLAYERS);
